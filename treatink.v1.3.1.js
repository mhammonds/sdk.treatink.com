 ...
 * 
 * Usage:
 * <script src="https://sdk.treatink.com/treatink.v1.3.0.js"></script>
 * <script>
 *   TreatInk.init({
 *     platform: 'shopify',
...
(function(window, document) {
  ...
  const TreatInk = {
    ...
    _setupEventListeners: function() {
      ...
      // Personalize button click (direct binding + prevent bubbling)
      const personalizeBtn = document.querySelector('[data-treatink-personalize="true"]');
      if (personalizeBtn) {
        const handler = (e) => {
          // Prevent theme/form click handlers from firing
          e.preventDefault();
          e.stopPropagation();
        ...
        personalizeBtn.addEventListener('click', handler, false);

        // Extra safety: some themes bind to mousedown/touchstart instead of click
        personalizeBtn.addEventListener('mousedown', (e) => { e.stopPropagation(); }, false);
        personalizeBtn.addEventListener('touchstart', (e) => { e.stopPropagation(); }, { passive: true });
      }
      ...
      window.addEventListener('message', (event) => {
        ...
        if (event.data.type === 'treatink_artwork_saved') {
          ...
          if (this.config.onPersonalizationComplete) {
            this.config.onPersonalizationComplete({ sessionUuid });
          }
        }
